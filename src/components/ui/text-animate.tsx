  ...
  rollIn: {
    ...
    child: {
      ...
      visible: {
        ...
        transition: {
          duration: 0.9,
          ease: [0.65, 0, 0.75, 0.65], // Great! Swift Beginning, Prolonged Ease, Quick Finish
...
const TextAnimate: FC<Props> = ({
  ...
  const isInView = useInView(ref, { once: false }) // Animate on every viewport entry
        ...
        {text.split(" ").map((word, index) => {
          return (
            <motion.span
              ...
              variants={container}
              transition={{
                delayChildren: index * 0.3, // smaller delay to keep animation responsive
